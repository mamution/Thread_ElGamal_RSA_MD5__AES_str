...
int main() {
	...
	send(sock, buff, strlen(buff), 0);
	Sleep(1000);
	cout << endl << "-------------------------------初始化RSA密钥---------------------------------" << endl;
	cout << "--请输入你的选择：\n"
		<<"\t1、创建两个大素数，并保存到txt文件中。\n"
		<<"\t2、从上次创建的txt文件中导入两个大素数。\n";
	int select;
	cin >> select;
	getchar();
	if (select==1)
	...
	// RSA公钥签名
	cout << endl << "----------------------将RSA公钥送往Trent(CA)进行数字签名--------------------" << endl;
	...
	// 发送RSA公钥给数字签名server
	cout << "--发送RSA公钥给数字签名Server Trent(CA)\n";
	memset(buff, 0, sizeof(buff));
	strcpy(buff, RSA_e_N.data());
	send(sock_ca, buff, strlen(buff), 0);
	Sleep(1000);
	...
	while (1) {
		...
		if (ret > 0) {
			...
			if (strcmp(buff, AES_Key) == 0) {
				...
				buff[ret] = '\0';
				string str(buff);
				cout << endl << "----------------------------------收到RSA加密的AES密钥-----------------------------" << endl;
				string key_m_tmp;//解密后接受明文的容器
				RSAdecode(rsa, str, key_m_tmp);
				string2byte(key_m_tmp, key);
				cout << "--RSA解密后的AES种子密钥是:" << endl;
				for (int i = 0;i < 16;i++) {
				...
				Sleep(1000);
				cout << "--发送AES密钥被确认收到消息" << endl;
			}
			else if (strcmp(buff, AES_Key_Successfully_Accepted) == 0) {
				cout << endl << "-----------------------------收到AES密钥被确认收到的确认消息---------------------------" << endl;
				unique_lock<mutex> lock(mtx);
